...
# ---------------------------------------------------------------------------
# Added for Parameter Transfer tool: generic parameter-conversion helpers.
# Unlike get_number_spec()/get_text_spec() (which hard-code ONE specific
# data type), these work with ANY Definition's data type generically - so
# a tool that copies an EXISTING project parameter's type doesn't need a
# brand new per-type helper added every time Trung has a Length/Integer/
# YesNo/etc parameter to convert.
# ---------------------------------------------------------------------------

def get_definition_spec(definition):
    """Return whatever data-type identifier `definition` uses to describe
    its own value type, in whichever form this Revit version expects
    (ForgeTypeId via GetDataType() on Revit 2022+, or the legacy
    ParameterType enum on older Revit). Used to create a NEW Definition
    with the exact same underlying data type as an existing one."""
    try:
...
def get_spec_label(definition):
    """Human-readable data type label for `definition` (e.g. 'Text',
    'Number', 'Length', 'Integer', 'Yes/No'), across Revit versions."""
    from Autodesk.Revit.DB import LabelUtils
    try:
...
def get_definition_group_raw(definition):
    """Return whatever raw parameter-group value `definition` is
    CURRENTLY bound under, in the exact form doc.ParameterBindings.
    Insert() expects on this Revit version (GroupTypeId on 2022+,
    BuiltInParameterGroup on older Revit) - so a new binding can PRESERVE
    the same group as an existing one, instead of hard-coding Dimensions/
    Data like get_dimensions_param_group()/get_data_param_group() do."""
    try:
...
def find_binding_by_name(doc, name):
    """Return (definition, binding) currently bound under `name` in
    doc.ParameterBindings, or (None, None) if nothing is bound under that
    name. Revit does not allow two different bound Definitions to share
    the same name in one project, so matching by name is a safe, unique
    lookup (same pattern as the existing find_wall_bound_definition() in
    Wall Top Elevation, generalized to any name instead of only Walls)."""
    it = doc.ParameterBindings.ForwardIterator()
    it.Reset()
    while it.MoveNext():
        definition = it.Key
...
def ensure_pynbt_shared_param_file(app):
    """Point Application.SharedParametersFilename at pyNBT's own shared
    parameter file (creating it if missing) - the SAME file/location Wall
    Top Elevation uses (%APPDATA%\\pyNBT\\pyNBT_SharedParameters.txt).
    Returns the ORIGINAL path so callers can restore it afterwards and
    not disturb whatever shared parameter file Trung's project was
    already pointed at.

    NOTE: Wall Top Elevation's script.py still has its own private copy
    of this exact logic (_ensure_shared_param_file), written before this
    shared version existed - it was left untouched there rather than risk
    editing an already-shipped, working tool. Any NEW tool from now on
    should call THIS shared version instead of copy-pasting another local
    one."""
    import os
