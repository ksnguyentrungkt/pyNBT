...
def main():
    ...
    try:
        for old_bar, scaled_points, normal_h, part_val, part_storage in replacements:
            ...
            # v2.2.0: figure out this bar's own "ideal" detail level from its
            # own scaled curve length (same spacing rule as v2.1.0), then
            # build a descending ladder of fallback point counts to try if
            # that ideal level gets rejected by Revit.
            total_len_ft = 0.0
            ...
            if new_bar is None:
                if last_error:
                    ...
                    creation_errors.append(
                        'Bar Id {}: CreateFromCurves returned no element at every fallback '
                        'detail level (down to a straight line) - old bar left in place'.format(old_id))
                continue

            apply_partition(new_bar, part_val, part_storage)
            doc.Delete(old_id)
            created += 1
            if used_count < ideal_count:
